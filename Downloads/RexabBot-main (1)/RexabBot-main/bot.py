...
SOURCE_CODE_RE = re.compile(r"^[a-zA-Z0-9_-]{3,64}$")


def extract_start_parameter(text: Optional[str]) -> Optional[str]:
    if not text:
        return None
...
async def ensure_user(
    ...
    async with DB_POOL.acquire() as conn:
        await conn.execute(
            """
            INSERT INTO users (user_id, username, first_name, last_name, source_code, created_at, updated_at)
            VALUES ($1, $2, $3, $4, $5, NOW(), NOW())
            ON CONFLICT (user_id) DO UPDATE SET
                username = EXCLUDED.username,
                first_name = EXCLUDED.first_name,
                last_name = EXCLUDED.last_name,
                source_code = COALESCE(EXCLUDED.source_code, users.source_code),
                updated_at = NOW()
            """,
            user_id, username, first_name, last_name, source_code
        )


async def get_user_source_code(user_id: int) -> Optional[str]:
    async with DB_POOL.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT source_code FROM users WHERE user_id = $1",
            user_id
        )
        return row["source_code"] if row else None


async def get_source_by_code(code: str) -> Optional[asyncpg.Record]:
    async with DB_POOL.acquire() as conn:
        return await conn.fetchrow(
            "SELECT code, title, description FROM sources WHERE code = $1",
            code
        )


async def list_sources() -> List[asyncpg.Record]:
    async with DB_POOL.acquire() as conn:
        return await conn.fetch(
            "SELECT code, title, description FROM sources ORDER BY created_at DESC"
        )


async def create_source(code: str, title: str, description: Optional[str]) -> bool:
    async with DB_POOL.acquire() as conn:
        exists = await conn.fetchrow("SELECT 1 FROM sources WHERE code = $1", code)
        if exists:
            return False
        await conn.execute(
            """
            INSERT INTO sources (code, title, description, created_at, updated_at)
...
async def update_source_title(code: str, title: str) -> bool:
    async with DB_POOL.acquire() as conn:
        result = await conn.execute(
            "UPDATE sources SET title = $2, updated_at = NOW() WHERE code = $1",
            code, title
        )
        return result.endswith("1")


async def update_source_description(code: str, description: Optional[str]) -> bool:
    async with DB_POOL.acquire() as conn:
        result = await conn.execute(
            "UPDATE sources SET description = $2, updated_at = NOW() WHERE code = $1",
            code, description
        )
        return result.endswith("1")


async def delete_source(code: str) -> bool:
    async with DB_POOL.acquire() as conn:
        result = await conn.execute("DELETE FROM sources WHERE code = $1", code)
        return result.endswith("1")


async def get_source_label(source_code: Optional[str]) -> Optional[str]:
    if not source_code:
        return None
    source = await get_source_by_code(source_code)
    if source:
        return f"{source['title']} ({source['code']})"
    return source_code
...
async def save_request(
    ...
    async with DB_POOL.acquire() as conn:
        if source_code is None:
            source_code = await get_user_source_code(user_id)
        await conn.execute(
            """
            INSERT INTO requests (user_id, scenario, topic, description, source_code, created_at, status)
            VALUES ($1, $2, $3, $4, $5, NOW(), 'new')
            """,
            user_id, scenario, topic, description, source_code
        )
...
async def save_request_with_id(
    user_id: int,
    scenario: str,
    topic: Optional[str],
    description: str,
...
async def create_bitrix_lead(
    ...
    title = f"[PHUKET_BOT] {scenario}"
    if topic:
        title += f" - {topic}"

    if source_code is None:
        source_code = await get_user_source_code(user_id)
    source_label = await get_source_label(source_code)
    ...
    if source_label:
        comments += f"Источник: {source_label}\n"
    ...
    payload = {
        "fields": {
            ...
            "COMMENTS": comments,
            "SOURCE_ID": "TELEGRAM",
            "SOURCE_DESCRIPTION": f"Telegram Bot - {scenario}" + (f" | Источник: {source_label}" if source_label else ""),
            "OPENED": "Y",
...
def kb_main_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="❗️ Экстренная помощь", callback_data="menu:emergency")],
...
def kb_services_list() -> InlineKeyboardMarkup:
    ...
    rows = [
        [InlineKeyboardButton(text=item.get("name", key), callback_data=f"svc:{key}")]
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    await state.clear()
    if message.from_user:
        raw_source = extract_start_parameter(message.text)
        source_code = raw_source if (raw_source and SOURCE_CODE_RE.match(raw_source)) else None
        await ensure_user(
...
async def menu_emergency(call: CallbackQuery) -> None:
    await call.message.edit_text(TEXTS.get("emergency", ""), reply_markup=kb_back_to_main())
    await call.answer()
...
async def cmd_admin(message: Message) -> None:
    ...
    admin_kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="� Источники (QR/ссылки)", callback_data="admin:sources")],
    ...
    if action == "stats":
        ...
        async with DB_POOL.acquire() as conn:
            users = await conn.fetch(
                ...
                "GROUP BY u.user_id, u.username, u.first_name, u.last_name, u.created_at, u.source_code "
                "ORDER BY u.created_at DESC"
            )
            
            requests = await conn.fetch(
                "SELECT r.id, r.user_id, u.username, r.scenario, r.topic, r.description, "
                "r.source_code, r.created_at, r.status "
                "FROM requests r JOIN users u ON r.user_id = u.user_id "
        ...
        users_writer.writerow(['User ID', 'Username', 'First Name', 'Last Name', 'Source Code', 'Created At', 'Requests Count'])
        for user in users:
        ...
        requests_writer.writerow(['Request ID', 'User ID', 'Username', 'Scenario', 'Topic', 'Description', 'Source Code', 'Created At', 'Status'])
        for req in requests:
        ...
        edit_kb = InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [InlineKeyboardButton(text="❗️ Экстренная помощь", callback_data="edit:emergency")],
        ...
        if not sources:
            text = "📎 <b>Источники (QR/ссылки)</b>\n\nПока нет источников."
            sources_kb = InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="➕ Создать источник", callback_data="source:add")],
                    [InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:back")],
                ]
            )
        else:
            ...
            for src in sources:
                rows.append([
                    InlineKeyboardButton(
                        text=f"✏️ {src['title']} ({src['code']})",
                        callback_data=f"source:edit:{src['code']}"
                    )
                ])
            rows.append([InlineKeyboardButton(text="➕ Создать источник", callback_data="source:add")])
            rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:back")])
            sources_kb = InlineKeyboardMarkup(inline_keyboard=rows)
        await call.message.edit_text(text, reply_markup=sources_kb)
        await call.answer()
        return
...
async def main() -> None:
    ...
    dp.message.register(admin_handlers.handle_text_edit, admin_handlers.AdminEditState.editing_emergency)
    ...
    dp.callback_query.register(admin_handlers.handle_text_edit, admin_handlers.AdminEditState.editing_bitrix_webhook)

    # Источники (QR/ссылки)
    dp.callback_query.register(source_handlers.source_callback, F.data.startswith("source:"))
    dp.message.register(source_handlers.handle_source_text, source_handlers.SourceState.adding_title)
    dp.message.register(source_handlers.handle_source_text, source_handlers.SourceState.adding_code)
    dp.message.register(source_handlers.handle_source_text, source_handlers.SourceState.adding_desc)
    dp.message.register(source_handlers.handle_source_text, source_handlers.SourceState.editing_title)
    dp.message.register(source_handlers.handle_source_text, source_handlers.SourceState.editing_desc)
    ...
    dp.callback_query.register(menu_emergency, F.data == "menu:emergency")
