...
class AdminEditState(StatesGroup):
    ...
    editing_emergency = State()
    editing_service = State()
    editing_faq_question = State()
    editing_faq_answer = State()
    adding_source_title = State()
    adding_source_code = State()
    adding_source_desc = State()
    editing_source_title = State()
    editing_source_desc = State()
    adding_admin = State()
...
async def edit_text_callback(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if action == "welcome":
        ...
        texts = admin_panel.load_texts_file()
        current_text = texts.get("emergency", "")

        cancel_kb = InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="❌ Отмена", callback_data="admin:edit_texts")]]
        )
        await call.message.edit_text(
            f"❗️ <b>Редактирование экстренной помощи</b>\n\n"
            f"<b>Текущий текст:</b>\n<code>{current_text}</code>\n\n"
            f"📝 Отправьте новый текст одним сообщением:",
            reply_markup=cancel_kb
        )
        await state.set_state(AdminEditState.editing_emergency)
        await state.update_data(editing_section="emergency")
        await call.answer()
    
    elif action == "services":
        texts = admin_panel.load_texts_file()
        service_items = texts.get("services", {}).get("items", {})
        services_kb_buttons = [
            [InlineKeyboardButton(text=item.get("name", key), callback_data=f"edit_svc:{key}")]
            for key, item in service_items.items()
        ]
        services_kb_buttons.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:edit_texts")])
        services_kb = InlineKeyboardMarkup(inline_keyboard=services_kb_buttons)
        await call.message.edit_text(
...
async def handle_text_edit(message: Message, state: FSMContext) -> None:
    ...
    elif current_state == AdminEditState.editing_emergency:
        new_text = message.text.strip()
        if admin_panel.update_emergency_text(new_text):
            back_kb = InlineKeyboardMarkup(
                inline_keyboard=[[InlineKeyboardButton(text="🏠 Вернуться в меню", callback_data="admin:back")]]
            )
            await message.answer(
                "✅ <b>Экстренная помощь обновлена!</b>\n\n"
                "Изменения вступят в силу после перезапуска бота.",
                reply_markup=back_kb
            )
        else:
